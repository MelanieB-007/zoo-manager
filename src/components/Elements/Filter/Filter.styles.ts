...
export const SelectHeader = styled.div<{ $isOpen: boolean }>`
  ...
  border: 2px solid
    ${(props) =>
      props.$isOpen
        ? "${({ theme }) => theme.colors.brand.greenLighter}"
        : "#e0e7d5"};
  border-radius: 12px;
  background: ${({ theme }) => theme.colors.ui.surface};
  cursor: pointer;
  display: flex;
  ...
  &:hover {
    border-color: ${({ theme }) => theme.colors.brand.greenLighter};
  }
`;
...
export const Label = styled.span`
  font-weight: 500;
  color: ${({ theme }) => theme.colors.brand.petrolGreenDark};
`;
...
export const Option = styled.div`
  ...
  &:hover {
    background: ${({ theme }) => theme.colors.brand.whiteLight};
  }
`;
